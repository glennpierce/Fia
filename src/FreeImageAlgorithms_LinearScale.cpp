...
template<class Tdst> FIBITMAP* STRETCH<Tdst>::StretchImageToType(FIBITMAP *src,
        FREE_IMAGE_TYPE type, double max)
{
    FIBITMAP *dst= NULL;
    unsigned x, y;
...
template<class Tdst> FIBITMAP* STRETCH<Tdst>::StretchImageAcrossRange(FIBITMAP *src, Tdst dst_min,
        Tdst dst_max)
{
    FIBITMAP *dst= NULL;
...
template<class Tsrc> FIBITMAP* LINEAR_SCALE<Tsrc>::convert(FIBITMAP *src, double min, double max,
