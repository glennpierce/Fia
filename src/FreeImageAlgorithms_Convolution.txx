    ...
    public:
        ...
        inline void Move(int x, int y)
        {
            this->current_src_ptr = (const_cast<Tsrc*>(this->src_first_pixel_address_ptr) + (y
                    + this->y_amount_to_image) * this->src_pitch_in_pixels + x_amount_to_image + x);
            
            this->current_src_center_ptr = this->current_src_ptr + (y_radius
                    * this->src_pitch_in_pixels) - x_radius;
        }
...
template<typename Tsrc> Kernel<Tsrc>::Kernel(FIABITMAP* src, int x_radius, int y_radius,
    ...
    xborder(src->xborder), yborder(src->yborder), x_radius(x_radius), y_radius(y_radius),
            divider(divider), src_image_width(FreeImage_GetWidth(src->fib)),
            src_image_height(FreeImage_GetHeight(src->fib)), kernel_width(x_radius * 2 + 1),
            kernel_height(y_radius * 2 + 1), x_reminder(kernel_width % BLOCKSIZE),
            y_reminder(kernel_height % BLOCKSIZE), x_max_block_size((kernel_width / BLOCKSIZE)
                    * BLOCKSIZE), y_max_block_size((kernel_height / BLOCKSIZE) * BLOCKSIZE),
            src_pitch_in_pixels(FreeImage_GetPitch(src->fib) / sizeof(Tsrc)),
            src_image_type(FreeImage_GetImageType(src->fib)), values(values)
{
...
template<typename Tsrc> inline void Kernel<Tsrc>::ConvolveKernelRow(KernelIterator<Tsrc> &iterator)
{
...
template<typename Tsrc> inline double Kernel<Tsrc>::AddKernelRow(KernelIterator<Tsrc> &iterator)
{
    register double *tmp;
...
template<typename Tsrc> inline void Kernel<Tsrc>::CorrelateKernelRow(KernelIterator<Tsrc> &iterator,
        double avg)
{
    register double *tmp, *kernel_ptr;
    ...
    for (register int col=0; col < x_max_block_size; col+=BLOCKSIZE) {
        ...
        this->sum += (var0 * (kernel_ptr[0] - kavg) + var1 * (kernel_ptr[1] - kavg) + var2
                * (kernel_ptr[2] - kavg) + var3 * (kernel_ptr[3] - kavg) + var4 * (kernel_ptr[4]
                - kavg) + var5 * (kernel_ptr[5] - kavg) + var6 * (kernel_ptr[6] - kavg) + var7
                * (kernel_ptr[7] - kavg));
        
        this->correlation_sum_squared += ((var0 * var0) + (var1 * var1) + (var2 * var2) + (var3
                * var3) + (var4 * var4) + (var5 * var5) + (var6 * var6) + (var7 * var7));
    }
    ...
    switch (x_reminder)
    {
        ...
        case 7:
        {
            this->sum += ((tmp[6] - avg) * (kernel_ptr[6] - kavg) + (tmp[5] - avg) * (kernel_ptr[5]
                    - kavg) + (tmp[4] - avg) * (kernel_ptr[4] - kavg) + (tmp[3] - avg)
                    * (kernel_ptr[3] - kavg) + (tmp[2] - avg) * (kernel_ptr[2] - kavg) + (tmp[1]
                    - avg) * (kernel_ptr[1] - kavg) + (tmp[0] - avg) * (kernel_ptr[0] - kavg));
            
            this->correlation_sum_squared += ((var6 * var6) + (var5 * var5) + (var4 * var4) + (var3
                    * var3) + (var2 * var2) + (var1 * var1) + (var0 * var0));
        ...
        case 6:
        {
            this->sum += ((tmp[5] - avg) * (kernel_ptr[5] - kavg) + (tmp[4] - avg) * (kernel_ptr[4]
                    - kavg) + (tmp[3] - avg) * (kernel_ptr[3] - kavg) + (tmp[2] - avg)
                    * (kernel_ptr[2] - kavg) + (tmp[1] - avg) * (kernel_ptr[1] - kavg) + (tmp[0]
                    - avg) * (kernel_ptr[0] - kavg));
            
            this->correlation_sum_squared += ((var5 * var5) + (var4 * var4) + (var3 * var3) + (var2
                    * var2) + (var1 * var1) + (var0 * var0));
        ...
        case 5:
        {
            this->sum += ((tmp[4] - avg) * (kernel_ptr[4] - kavg) + (tmp[3] - avg) * (kernel_ptr[3]
                    - kavg) + (tmp[2] - avg) * (kernel_ptr[2] - kavg) + (tmp[1] - avg)
                    * (kernel_ptr[1] - kavg) + (tmp[0] - avg) * (kernel_ptr[0] - kavg));
        ...
        case 4:
        {
            this->sum += ((tmp[3] - avg) * (kernel_ptr[3] - kavg) + (tmp[2] - avg) * (kernel_ptr[2]
                    - kavg) + (tmp[1] - avg) * (kernel_ptr[1] - kavg) + (tmp[0] - avg)
                    * (kernel_ptr[0] - kavg));
        ...
        case 3:
        {
            this->sum += ((tmp[2] - avg) * (kernel_ptr[2] - kavg) + (tmp[1] - avg) * (kernel_ptr[1]
                    - kavg) + (tmp[0] - avg) * (kernel_ptr[0] - kavg));
