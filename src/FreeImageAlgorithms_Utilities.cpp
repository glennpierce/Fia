...
template<class Tsrc> void FIND_MINMAX_FOR_DIB<Tsrc>::find(FIBITMAP *src, double *min, double *max)
{
    if (!src) {
        return;
    }
...
template<class Tsrc> void FIND_MINMAX_FOR_DIB<Tsrc>::find_max_xy(FIBITMAP *src, double *max,
        FIAPOINT *pt)
{
    if (!src) {
        return;
...
template <class T> int GetPixelValuesForLine(FIBITMAP *src, FIAPOINT p1, FIAPOINT p2, T *values)
{
    T value;
