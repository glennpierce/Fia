...
template<class Tsrc> FIBITMAP* FFT2D<Tsrc>::TransformStandardToComplexImage(FIBITMAP *src,
        int inverse, int shift)
{
...
static FIBITMAP* TransformComplexToComplexImage(FIBITMAP *src, int inverse, int shift)
{
...
static FIBITMAP* ConvertComplexImageToAbsoluteValued(FIBITMAP *src, bool squared)
{
    FIBITMAP *dst = NULL;
    unsigned x, y;
    ...
    if (squared) {
        ...
        for (y = 0; y < height; y++) {
            ...
            for (x=0; x < width; x++) {
                ...
                *(dst_bits + x) = (double) (pow( (double)((src_bits + x)->r), (double) 2.0) + pow( (double)((src_bits + x)->i), (double) 2.0));
        ...
        for (y = 0; y < height; y++) {
            ...
            for (x=0; x < width; x++) {
                ...
                *(dst_bits + x) = (double) sqrt(pow( (double)((src_bits + x)->r), (double) 2.0) + pow( (double)((src_bits + x)->i), (double) 2.0));
            }
        }
    }
