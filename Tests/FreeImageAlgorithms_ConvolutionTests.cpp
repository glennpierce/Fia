...
static void
TestFIA_CorrelateKernelFilterTest(CuTest* tc)
{
    double max;
    const char *file1 = TEST_DATA_DIR "/CorrelationSections/spice-small1.png";
    const char *file2 = TEST_DATA_DIR "/CorrelationSections/spice-small2.png";

    FIBITMAP *src1 = FIA_LoadFIBFromFile(file1);
    FIBITMAP *src2 = FIA_LoadFIBFromFile(file2);
	FIBITMAP *dst = FreeImage_Allocate(300, 100, 24, 0, 0, 0);

        CuAssertTrue(tc, src1 != NULL);
        CuAssertTrue(tc, src2 != NULL);
	CuAssertTrue(tc, dst != NULL);

	FIA_InPlaceConvertTo24Bit(&src1);
	FIA_InPlaceConvertTo24Bit(&src2);

        PROFILE_START("TestFIA_CorrelateFilterTest");

        FIAPOINT pt;

        if(FIA_KernelCorrelateImages(src1, src2, NULL, &pt, &max) == FIA_ERROR) {
            PROFILE_STOP("TestFIA_CorrelateFilterTest");
            goto TEST_ERROR;
        }
    ...
    if(FIA_Paste(dst, src1, 15, 15) == 0) {
        printf("Paste failed for TestFIA_CorrelateFilterTest. Trying to paste at %d, %d\n", 15, 15);
    }

    if(FIA_Paste(dst, src2, 15 + pt.x, 15 + pt.y) == 0) {
        printf("Paste failed for TestFIA_CorrelateFilterTest. Trying to paste at %d, %d\n", 15 + pt.x, 15 + pt.y);
    }

        FIA_SaveFIBToFile(dst, TEST_DATA_OUTPUT_DIR  "/Convolution/kernel-small.bmp", BIT24);
...
CuGetFreeImageAlgorithmsConvolutionSuite(void)
{
	...
	SUITE_ADD_TEST(suite, TestFIA_CorrelateKernelFilterTest);
